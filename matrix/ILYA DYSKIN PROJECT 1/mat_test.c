...
int main()
{
    ...
    m_fileread=mat_frd("sample_matrix.txt");
    
    mat_pr("Matrix from the file", m_fileread);
    
    matrix_t result = mat_new(4,3);
    merr = mat_trans(m_fileread, result);
    if(merr==0)
        mat_pr("TRANSPOSE", result);
...
* read a matrix from disk given a string filename:
